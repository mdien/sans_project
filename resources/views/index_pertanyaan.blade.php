    ...
    @foreach($pertanyaan as $tanya)
        <div class="col">
            ...
            <div class="card-body">
                ...
                <h6 class="card-subtitle mb-2 text-muted">Pertanyaan #{{$tanya->id}}</h6>
                <p class="card-text">{{$tanya->isi}}</p>
                <div class="btn-group" role="group" aria-label="Basic example">
                <form action="{{route('pertanyaan.edit',$tanya->id)}}">
                    <button class="btn btn-primary mr-2">Edit</button>
                </form>
                <form action="{{route('pertanyaan.show',$tanya->id)}}">
                    <button class="btn btn-success mr-2">Show</button>
                </form>
                <form action="{{route('pertanyaan.destroy',$tanya->id)}}" method="POST">
                    {{csrf_field()}}
                    {{ method_field('DELETE') }}
